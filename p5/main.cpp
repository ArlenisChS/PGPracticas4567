...
private:
	...
	std::shared_ptr<ListBoxWidget<>> shader_selector;
...
void MyRender::setup() {
	...
	gouraud.loadFiles("../p5/gouraud");
	gouraud.compile();

	// Este shader se encarga de calcular la iluminación, usando
	// el algoritmo de Phong 
	phong.loadFiles("../p5/phong");
	phong.compile();
...
void MyRender::buildGUI() {
	...
	// Este widget permite seleccionar el shader que se utilizará
	const std::string label_shader_selector = "Shader Selector";
	const Strings items_shader_selector = { "Gouraud", "Phong" };
	shader_selector = std::make_shared<ListBoxWidget<>>(label_shader_selector, items_shader_selector, 0);
	panel->addWidget(shader_selector);
